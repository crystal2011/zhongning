...
class ad {
	...
	function set_ad($ad) {
        ...
        if(!isset($ad['image_src'])) $ad['image_src']= '';
        if(!isset($ad['flash_src'])) $ad['flash_src']= '';
        if(!isset($ad['code'])) $ad['code']= '';
    ...
    function getAdList($condition='1=1',$field='*'){
        $result = $this->db->query("select {$field} from {$this->table} where $condition");
        $arr = array();
        while($r=$this->db->fetch_array($result)){
            $arr[] = $r;
        }
    ...
    function getAdAllText($ad){
        ...
        if($list){
            foreach($list as $k=>$v){
                ...
                $text_url = $v['text_url'];
                $text_title = $v['text_title'];
                $str .= '<a href="'.$text_url.'" title="'.$text_title.'" target="_blank">'.$text_name.'</a>';
            }
        }
        return $str;
    }
    ...
    function moread($aid=array()){
        global $DT_TIME;
        $sid = implode(',',$aid);
        $result = $this->db->query("select a.pid,a.status,a.fromtime,a.totime,a.image_src,a.title,a.image_url,ap.width,ap.height from {$this->table} a inner join {$this->table_place} ap on ap.pid = a.pid where ap.pid in ($sid)");

        $list = array();
        while($r=$this->db->fetch_array($result)){

            if($r['status']!=3 || $r['fromtime']>$DT_TIME || $r['totime']<$DT_TIME) {
                $r['image_src'] = '广告失效';
                $r['title'] = '广告失效';
            }
            $list[$r['pid']] = $r;
