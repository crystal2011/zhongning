...
class company {
	...
	function get_list($condition, $order = 'userid DESC', $cache = '') {
		...
		if($page > 1 && $sum) {
			...
			$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->table_company} c inner join sx_member m on m.userid = c.userid WHERE $condition", $cache);
			$items = $r['num'];
		}
		$pages = defined('CATID') ? listpages(1, CATID, $items, $page, $pagesize, 10, $MOD['linkurl']) : pages($items, $page, $pagesize);
		if($items < 1) return array();
		$lists = array();
		$result = $this->db->query("SELECT c.company,c.areaid,m.username,m.userid FROM {$this->table_company} c inner join sx_member m on m.userid = c.userid WHERE $condition ORDER BY $order LIMIT $offset,$pagesize", $cache);
    ...
    function getUuserCompanyInfo($arr,$field='*'){
        if(!$arr) return array();
        $sUserid = '';
        foreach($arr as $k=>$v){
            $sUserid .= $v['userid'].',';
        ...
        $result = $this->db->query("select {$field} from {$this->table_company} where userid in ($sUserid)");
        $aCompany = array();
        while($r = $this->db->fetch_array($result)){
            $aCompany[$r['userid']] = $r;
        }
