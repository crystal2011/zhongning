...
class cai {
    ...
    function cai() {
        ...
        $this->moduleid = 10;
        $this->table = $db->pre.'know_cai';
        $this->table_data = $db->pre.'know_data_cai';
        $this->split = '';
        $this->db = &$db;
        $this->fields = array('catid','istop','level','userid','knowid','title','style','fee','credit','aid','process','hidden','introduce','addition','thumb','tag','status','hits','username','ask','addtime','totime','editor','edittime','ip','template', 'linkurl','filepath','note');
    }
    ...
    function set($post) {
        ...
        if($post['content'] && empty($post['introduce'])) $post['introduce'] = addslashes(get_intro($post['content'], isset($post['introduce_length']) && $post['introduce_length']?$post['introduce_length']:120));
    ...
    function getSchoolUser($arr,$field='*'){
        if(!$arr) return array();
        $sKnowid = '';
        foreach($arr as $k=>$v){
            $sKnowid .= $v['knowid'].',';
        }
        $sKnowid = substr($sKnowid,0,-1);
        $result = $this->db->query("select {$field} from {$this->db->pre}know where itemid in ($sKnowid)");
        $list = array();
        while($r = $this->db->fetch_array($result)){
            $list[$r['itemid']] = $r;
        }
