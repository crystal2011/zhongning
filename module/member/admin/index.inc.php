...
if(in_array($action, array('', 'check'))) {
	$sfields = array('按条件', '会员名', '手机号', '注册IP', '登录IP');
	$dfields = array('username','username', 'mobile', 'regip', 'loginip');
	$sorder  = array('结果排序方式', '注册时间降序', '注册时间升序', '修改时间降序', '修改时间升序', '登录时间降序', '登录时间升序', '登录次数降序', '登录次数升序');
	$dorder  = array('userid DESC', 'regtime DESC', 'regtime ASC', 'edittime DESC', 'edittime ASC', 'logintime DESC', 'logintime ASC', 'logintimes DESC', 'logintimes ASC');
...
switch($action) {
	...
	case 'show':
        ...
        if($userid){
            $user = $do->get_one("userid='".$userid."'");
        }else{
            $user = $do->get_one("username='".$username."'");
        }
