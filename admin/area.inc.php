...
$menus = array (
    ...
    array('热门城市', '?file='.$file.'&action=hot'),
...
switch($action) {
    ...
    case 'deletehot':
        $areaid = isset($areaid)?intval($areaid):0;
        $db->query("update {$table} set ishot = 0 WHERE areaid = ".$areaid);
        dmsg('删除成功', $forward);
    break;
	default:
		$DAREA = array();
...
class area {
	...
	function update($area) {
		...
		foreach($area as $k=>$v) {
            ...
            $v['ishot'] = intval($v['ishot'])?1:0;
			$this->db->query("UPDATE {$this->table} SET areaname='$v[areaname]',parentid='$v[parentid]',listorder='$v[listorder]',ishot='$v[ishot]' WHERE areaid=$k");
		}
		cache_area();
		return true;
	}
