...
<td>&nbsp;<a href="?file=<?php echo $file;?>&parentid=<?php echo $v['areaid'];?>"><?php echo $v['childs'];?></a></td>
    <td><input name="area[<?php echo $v['areaid'];?>][ishot]" type="checkbox"  <?php echo $v['ishot']?'checked':'';?> value="1" /></td>
<td>
...
<?php if($parentid) {?>
<input type="button" value="上级地区" class="btn" onclick="Go('?file=<?php echo $file;?>&parentid=<?php echo $AREA[$parentid]['parentid'];?>');"/>
<?php }?>
</div>
</form>
...
<div style="float:left;padding:10px;">
	...
	<tr>
	<td><input type="submit" value="删除地区" class="btn" onclick="if(confirm('确定要删除选中地区吗？此操作将不可撤销')){this.form.action='?file=<?php echo $file;?>&action=delete&areaid='+Dd('aid').value;}else{return false;}"/></td>
	</tr>
	</table>
</div>
</td>
