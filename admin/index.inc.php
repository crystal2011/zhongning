...
switch($action) {
	...
	case 'main':
		if($submit) {
			...
			$user = $db->get_one("SELECT loginip,logintime,logintimes FROM {$DT_PRE}admin_user WHERE adminuid=$_adminuserid");
            $note = DT_ROOT.'/file/user/'.dalloc($_adminuserid).'/'.$_adminuserid.'/note.php';
			$note = file_get($note);
			if($note) {
				$note = substr($note, 13);
			} else {
