...
function status_show($status){
    $arr = array('回收站','拒绝','待审核','正常','屏蔽');
    return $arr[$status];
}

function is_can_edit($info){
    global $_userid;
    if(!$info){
        return '信息不存在';
    }
...
function getcity(){
    ...
    $cookiecity = get_cookie('cookiecity');
    if(!$cookiecity) return array();

    $isHas = false;
    $aCity = cache_read('city.php');
    $aCityarr =  isset($aCity['city'][$cookiecity])?$aCity['city'][$cookiecity]:array();
    if($aCityarr){
        set_cookie('cookiecity',$cookiecity,time()+3600*24*30);
    }
    return $aCityarr;
}
...
function setcity($areaid){
    $aCity = cache_read('city.php');
    if($areaid==0 || (isset($aCity['city'][$areaid]) && isset($aCity['city'][$areaid]))){
        set_cookie('cookiecity',$areaid,time()+3600*24*30);
    }
}

function is_number($val,$max=7){
    return preg_match('/^0|[1-9][0-9]{0,'.$max.'}$/',$val)?true:false;
}

function is_price($val){
    return preg_match('/^(0|[1-9][0-9]{0,8})(\.[0-9]{1,2})?$/',$val)?true:false;
}

function getCacheList($moduleid,$catid=0){
    $aCacheCatList= cache_read('catelist-'.$moduleid.'.php');
    return isset($aCacheCatList[$catid])?$aCacheCatList[$catid]:array();
}

function is_length($val,$min=0,$max=0){
    $val = dhtmlspecialchars(daddslashes(dstripslashes($val)));
...
function showArea($areainfo){
    $areaid = $areainfo?($areainfo['child']?$areainfo['areaid']:$areainfo['parentid']):0;
    return get_mainarea($areaid);
}

function showAllArea($areainfo){
    $aArea = array();
    if($areainfo){

        $sas = explode(',',$areainfo['arrparentid'].','.$areainfo['areaid']);
        foreach($sas as $k=>$v){
            $aArea[] = get_mainarea($v);
        }
    }else{
        $aArea = array(get_mainarea(0));
    }
...
function showCat($catinfo,$moduleid){
    $catid = $catinfo?($catinfo['child']?$catinfo['catid']:$catinfo['parentid']):0;
    return getCacheList($moduleid,$catid);
}
...
/**
 * 创建公共token
 * 保存到session 和 cookie当中
 */
function set_token(){
    global $DT_TIME,$session;
    $session or $session = new dsession();
    $commontoken = create_token('commontoken');
    set_cookie('commontoken',$commontoken,$DT_TIME + 86400*7);
    return $commontoken;
}
 ...
 * @return string
 */
function create_token($str,$itemid=0){
    global $DT_IP;
    $token = md5($str.md5(time().'token'.DT_KEY).$DT_IP);
    $returnToken = '';
    if($itemid){
        if(!empty($_SESSION['form_token'][$str]) && count($_SESSION['form_token'][$str])>=5 && empty($_SESSION['form_token'][$str][$itemid])){
            array_shift($_SESSION['form_token'][$str]);
        }
        $_SESSION['form_token'][$str][$itemid] = $token;
        $returnToken = $_SESSION['form_token'][$str][$itemid];
    }else{
        $_SESSION['form_token'][$str] = $token;
        $returnToken = $_SESSION['form_token'][$str];
    }
...
function check_token(){
    global $DT_TIME,$session;
    $session or $session = new dsession();
    $commontoken = get_cookie('commontoken');
    if(check_tokens('commontoken',$commontoken)){
        return set_token();
    }else{
        return false;
    }
...
function check_tokens($str,$val,$itemid=0,$unset=true){
    if($itemid){
        $token = empty($_SESSION['form_token'][$str][$itemid])?0:$_SESSION['form_token'][$str][$itemid];
        if($token && $unset) unset ($_SESSION['form_token'][$str][$itemid]);
    }else{
        $token = empty($_SESSION['form_token'][$str])?0:$_SESSION['form_token'][$str];
        if($token && $unset) unset ($_SESSION['form_token'][$str]);
    }
    return empty($token) || empty($val) || $token != $val?false:true;
...
function horncode($type,$itemid){
    ...
    $oSpecial->codeStatus($itemid,$type);
}

/**
 * 检测预览功能     拒绝、待审核、正常  并且是该羊角会会员审核的
 * @param $type
 * @param $itemid
 */
function chcekHorn($info,$type){
    global $db,$_userid;
    if(!$info) return '信息不存在';
    if($info['status']!=2) return '信息不可预览';
    if(!$_userid)  return '请先登录';

    $specialinfo = $db->get_one("select itemid from {$db->pre}special where status = 3 and userid = ".$_userid);
    if(!$specialinfo) return '您不可以预览';

    $info = $db->get_one("select itemid from {$db->pre}code where codeid = ".$specialinfo['itemid']." and id = ".$info['itemid']." and status = 2 and type = ".$type);
    if(!$info) return '信息不可以预览';
 ...
 * 发布时 添加发布数量
 */
function addPublishs($userid){
    global $db;
    $db->query("update {$db->pre}member set infopublishs = infopublishs + 1 where userid = ".$userid);
}

/**
 * 浏览时 添加浏览数量
 */
function addHits($userid){
    global $db;
    $db->query("update {$db->pre}member set infohits = infohits + 1 where userid = ".$userid);
}

function wapforward(){
    global $forward,$CFG;
    return (empty($forward) || $forward==$CFG['url'] || $forward==DT_PATH)?$CFG['url'].'mobile/index.php':$forward;
}
