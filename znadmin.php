...
$_founder = $CFG['founderid'] == $_adminuserid ? $_adminuserid : 0;
$_catids = $_childs = '';
$_catid = $_child = array();
if($file != 'login') {
    if($_adminadmin < 1 ) msg('', '?file=login&forward='.urlencode($DT_URL));
	if(!admin_check()) {
		admin_log(1);
		$db->query("DELETE FROM {$db->pre}admin WHERE userid=$_adminuserid AND url='?".$DT_QST."'");
