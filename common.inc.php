...
$session = new dsession();
$destoon_auth or $destoon_auth = isset($_SESSION['auth'])?$_SESSION['auth']:'';  //如果没有cookie 则查询session
...
$admindestoon_auth = get_cookie('adminauth');
$_adminuserid = $_admin = 0;
$_adminusername = '';
if($admindestoon_auth) $admindestoon_auth = decrypt($admindestoon_auth, DT_KEY.'USER');
if($admindestoon_auth) {
    $admindestoon_auth = explode('|', $admindestoon_auth);
    $_adminuserid = isset($admindestoon_auth[0]) ? intval($admindestoon_auth[0]) : 0;
    if($_adminuserid) {
        $_adminpassword = isset($admindestoon_auth[1]) ? trim($admindestoon_auth[1]) : '';
        $USER = $db->get_one("SELECT username,password,loginip,admin FROM {$DT_PRE}admin_user WHERE adminuid=$_adminuserid");
        if($USER && $USER['password'] == $_adminpassword) {
            if($USER['loginip'] != $DT_IP && ($DT['ip_login'] == 2 || ($DT['ip_login'] == 1 && IN_ADMIN))) {
                $_adminuserid = 0; set_cookie('adminauth', '');
                dalert(lang('message->common_login', array($USER['loginip'])), DT_PATH);
            }
            $_adminusername = $USER['username'];
            $_adminpassword = $USER['password'];
            $_adminloginip = $USER['loginip'];
            $_adminadmin = $USER['admin'];
            $_adminloginip = $USER['loginip'];
        } else {
            $_adminuserid = 0;
            if($db->linked && !isset($swfupload) && strpos($_SERVER['HTTP_USER_AGENT'], 'Flash') === false) set_cookie('adminauth', '');
        }
        unset($admindestoon_auth, $USER, $admindestoon_auth, $_adminpassword);
    }
}
